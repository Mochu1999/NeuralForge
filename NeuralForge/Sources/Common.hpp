...
//vector<vector<T>> row major
template<typename T>
void print_(const std::string& name, const std::vector<std::vector<T>>& matrix) {
	std::stringstream ss;
	ss << name << ":  " << std::endl;
	ss << "    {{";  // Indent and start with double brace
	for (size_t i = 0; i < matrix.size(); ++i) {
		...
		ss << "}";  // Close the row
		if (i != matrix.size() - 1) {
			ss << ", " << std::endl << "     {";  // Indent and open brace for next row
		}
	}
	ss << "}" << std::endl << std::endl;  // Close outer brace and add two newlines
	std::cout << ss.str();
}
